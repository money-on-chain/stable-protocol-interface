...
const contractStatus = async (web3, dContracts, appMode) => {
    ...
    if (appMode === 'MoC') {
        ...
        listMethods = [
            [
                mocstate.options.address,
                mocstate.methods.getReserveTokenPrice().encodeABI(),
                'uint256'
            ], // 0
            [
                mocstate.options.address,
            ...
            [
                mocstate.options.address,
                mocstate.methods.absoluteMaxRiskPro().encodeABI(),
                'uint256'
            ], // 2
            [
                mocstate.options.address,
                mocstate.methods.maxRiskProx(BUCKET_X2).encodeABI(),
                'uint256'
            ], // 3
            [
                mocstate.options.address,
                mocstate.methods.absoluteMaxStableToken().encodeABI(),
                'uint256'
            ], // 4
            [
                mocstate.options.address,
                mocstate.methods.freeStableToken().encodeABI(),
                'uint256'
            ], // 5
            [
                mocstate.options.address,
            ...
            [
                mocstate.options.address,
                mocstate.methods.reserves().encodeABI(),
                'uint256'
            ], // 9
            [
                mocstate.options.address,
                mocstate.methods.getExponentalMovingAverage().encodeABI(),
                'uint256'
            ], // 10
            [
                mocstate.options.address,
            ...
            [
                mocstate.options.address,
                mocstate.methods.getBucketNReserve(BUCKET_C0).encodeABI(),
                'uint256'
            ], // 12
            [
                mocstate.options.address,
                mocstate.methods.getBucketNStableToken(BUCKET_C0).encodeABI(),
                'uint256'
            ], // 13
            [
                mocstate.options.address,
                mocstate.methods.getBucketNRiskPro(BUCKET_C0).encodeABI(),
                'uint256'
            ], // 14
            [
                mocstate.options.address,
                mocstate.methods.getBucketNReserve(BUCKET_X2).encodeABI(),
                'uint256'
            ], // 15
            [
                mocstate.options.address,
                mocstate.methods.getBucketNStableToken(BUCKET_X2).encodeABI(),
                'uint256'
            ], // 16
            [
                mocstate.options.address,
                mocstate.methods.getBucketNRiskPro(BUCKET_X2).encodeABI(),
                'uint256'
            ], // 17
            [
                mocstate.options.address,
            ...
            [
                mocstate.options.address,
                mocstate.methods.riskProTecPrice().encodeABI(),
                'uint256'
            ], // 22
            [
                mocstate.options.address,
                mocstate.methods.riskProUsdPrice().encodeABI(),
                'uint256'
            ], // 23
            [
                mocstate.options.address,
                mocstate.methods.riskProSpotDiscountRate().encodeABI(),
                'uint256'
            ], // 24
            [
                mocstate.options.address,
                mocstate.methods.maxRiskProWithDiscount().encodeABI(),
                'uint256'
            ], // 25
            [
                mocstate.options.address,
                mocstate.methods.riskProDiscountPrice().encodeABI(),
                'uint256'
            ], // 26
            [
                mocstate.options.address,
                mocstate.methods.bucketRiskProTecPrice(BUCKET_X2).encodeABI(),
                'uint256'
            ], // 27
            [
                mocstate.options.address,
                mocstate.methods.riskProxRiskProPrice(BUCKET_X2).encodeABI(),
                'uint256'
            ], // 28
            [
                mocinrate.options.address,
    ...
    if (appMode === 'MoC') {
        listMethodsRates = [
            [
                mocinrate.options.address,
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.MINT_BPRO_FEES_RBTC
                    )
                    .encodeABI(),
                'uint256'
            ], // 0
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.REDEEM_BPRO_FEES_RBTC
                    )
                    .encodeABI(),
                'uint256'
            ], // 1
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.MINT_DOC_FEES_RBTC
                    )
                    .encodeABI(),
                'uint256'
            ], // 2
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.REDEEM_DOC_FEES_RBTC
                    )
                    .encodeABI(),
                'uint256'
            ], // 3
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.MINT_BTCX_FEES_RBTC
                    )
                    .encodeABI(),
                'uint256'
            ], // 4
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.REDEEM_BTCX_FEES_RBTC
                    )
                    .encodeABI(),
                'uint256'
            ], // 5
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.MINT_BPRO_FEES_MOC
                    )
                    .encodeABI(),
                'uint256'
            ], // 6
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.REDEEM_BPRO_FEES_MOC
                    )
                    .encodeABI(),
                'uint256'
            ], // 7
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.MINT_DOC_FEES_MOC
                    )
                    .encodeABI(),
                'uint256'
            ], // 8
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.REDEEM_DOC_FEES_MOC
                    )
                    .encodeABI(),
                'uint256'
            ], // 9
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.MINT_BTCX_FEES_MOC
                    )
                    .encodeABI(),
                'uint256'
            ], // 10
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.REDEEM_BTCX_FEES_MOC
                    )
                    .encodeABI(),
                'uint256'
            ] // 11
        ...
        listMethodsRates = [
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.MINT_RISKPRO_FEES_RESERVE
                    )
                    .encodeABI(),
                'uint256'
            ], // 0
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes
            ...
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.MINT_RISKPRO_FEES_MOC
                    )
                    .encodeABI(),
                'uint256'
            ], // 6
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.REDEEM_RISKPRO_FEES_MOC
                    )
                    .encodeABI(),
                'uint256'
            ], // 7
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.MINT_STABLETOKEN_FEES_MOC
                    )
                    .encodeABI(),
                'uint256'
            ], // 8
            ...
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.MINT_RISKPROX_FEES_MOC
                    )
                    .encodeABI(),
                'uint256'
            ], // 10
            [
                ...
                mocinrate.methods
                    .commissionRatesByTxType(
                        dMocState.commissionRatesTypes.REDEEM_RISKPROX_FEES_MOC
                    )
                    .encodeABI(),
                'uint256'
            ] // 11
    ...
    const multicallResultRates = await multicall.methods
        .tryBlockAndAggregate(false, cleanListMethodsRates)
        .call();

    const listReturnDataRates = multicallResultRates[2].map((item, itemIndex) =>
        web3.eth.abi.decodeParameter(listMethods[itemIndex][2], item.returnData)
    );
    ...
    if (appMode === 'MoC') {
        ...
        commissionRates.REDEEM_STABLETOKEN_FEES_RESERVE =
            listReturnDataRates[3];
...
const calcCommission = async (
    ...
    switch (token) {
        case 'DOC':
            if (action === 'MINT') {
                reserveType =
                    dataContractStatus.commissionRatesTypes.MINT_DOC_FEES_RBTC;
                mocType =
                    dataContractStatus.commissionRatesTypes.MINT_DOC_FEES_MOC;
            } else {
                reserveType =
                    dataContractStatus.commissionRatesTypes
                        .REDEEM_DOC_FEES_RBTC;
                mocType =
                    dataContractStatus.commissionRatesTypes.REDEEM_DOC_FEES_MOC;
            }
            break;
        case 'BPRO':
            if (action === 'MINT') {
                reserveType =
                    dataContractStatus.commissionRatesTypes.MINT_BPRO_FEES_RBTC;
                mocType =
                    dataContractStatus.commissionRatesTypes.MINT_BPRO_FEES_MOC;
            } else {
                reserveType =
                    dataContractStatus.commissionRatesTypes
        ...
        case 'BTCX':
            if (action === 'MINT') {
                reserveType =
                    dataContractStatus.commissionRatesTypes.MINT_BTCX_FEES_RBTC;
                mocType =
                    dataContractStatus.commissionRatesTypes.MINT_BTCX_FEES_MOC;
            } else {
                reserveType =
                    dataContractStatus.commissionRatesTypes
