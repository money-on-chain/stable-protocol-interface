...
const redeemTP = async (
    ...
    if (new BigNumber(tpAmount).gt(userTPBalance))
        throw new Error('Insufficient TP user balance');
    ...
    if (new BigNumber(tpAmount).gt(tpAvailableToRedeem))
        throw new Error(
            `Insufficient ${tokens.RESERVE.name} available to redeem in contract`
        );
...
const mintTX = async (
    ...
    const mintInterest = await calcMintInterest(
        interfaceContext,
        reserveAmount
    );
...
export { mintTP, redeemTP, mintTC, redeemTC, mintTX, redeemTX };
