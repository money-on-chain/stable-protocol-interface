...
export function readJsonTableFastBtcPegOut(data_j) {
    ...
    const hash_id_cut =
        data_j.transferId !== undefined
            ? data_j.transferId?.slice(0, 5) +
              '...' +
              data_j.transferId?.slice(-4)
            : '--';
    ...
    const btcAmount =
        data_j.amountSatoshi !== undefined
            ? parseFloat(data_j.amountSatoshi / 100000000).toFixed(6)
            : 0;
    // const btcFee= (data_j.feeSatoshi!==undefined)? data_j.feeSatoshi : '--'
    const btcFee =
        data_j.feeSatoshi !== undefined
            ? parseFloat(data_j.feeSatoshi / 100000000).toFixed(6)
            : 0;
    const btcAddressCut =
        data_j.btcAddress !== undefined
            ? data_j.btcAddress?.slice(0, 5) +
              '...' +
              data_j.btcAddress?.slice(-4)
            : '--';
    const btcAddress =
        data_j.btcAddress !== undefined ? data_j.btcAddress : '--';
    const date = data_j.updated !== undefined ? data_j.updated : '--';
    const timestamp = data_j.timestamp !== undefined ? data_j.timestamp : '--';
    const transactionHash =
        data_j.transactionHash !== undefined ? data_j.transactionHash : '--';
    const transactionHashCut =
        data_j.transactionHash !== undefined
            ? data_j.transactionHash?.slice(0, 5) +
              '...' +
              data_j.transactionHash?.slice(-4)
            : '--';
    const blockNumber =
        data_j.blockNumber !== undefined ? data_j.blockNumber : '--';
    const rskAddress =
        data_j.rskAddress !== undefined ? data_j.rskAddress : '--';
    const rskAddressCut =
        data_j.rskAddress !== undefined
            ? data_j.rskAddress?.slice(0, 5) +
              '...' +
              data_j.rskAddress?.slice(-4)
            : '--';
    const transactionHashLastUpdated =
        data_j.transactionHashLastUpdated !== undefined
            ? data_j.transactionHashLastUpdated?.slice(0, 5) +
              '...' +
              data_j.transactionHashLastUpdated?.slice(-4)
            : '--';
