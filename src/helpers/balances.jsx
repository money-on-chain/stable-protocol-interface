...
const userTPBalance = (auth) => {
    if (auth.userBalanceData && auth.contractStatusData) {
        const tpBalance = new BigNumber(
            Web3.utils.fromWei(auth.userBalanceData['docBalance'])
        );
        const tpBalanceCollateral = tpBalance.div(
            new BigNumber(
                Web3.utils.fromWei(auth.contractStatusData.bitcoinPrice)
            )
        );
        return {
            normal: tpBalance,
...
const userTCBalance = (auth) => {
    if (auth.userBalanceData && auth.contractStatusData) {
        const tcBalance = new BigNumber(
            Web3.utils.fromWei(auth.userBalanceData.bproBalance)
        );
        const tcBalanceUsd = new BigNumber(
            Web3.utils.fromWei(auth.contractStatusData.bproPriceInUsd)
        ).multipliedBy(tcBalance);
        const tcBalanceCollateral = tcBalanceUsd.div(
            new BigNumber(
                Web3.utils.fromWei(auth.contractStatusData.bitcoinPrice)
            )
        );
...
const userTXBalance = (auth) => {
    if (auth.userBalanceData && auth.contractStatusData) {
        const txBalance = new BigNumber(
            Web3.utils.fromWei(auth.userBalanceData['bprox2Balance'])
        );
        const txBalanceCollateral = new BigNumber(
            Web3.utils.fromWei(auth.contractStatusData.bprox2PriceInRbtc)
        ).multipliedBy(txBalance);
        const txBalanceUsd = txBalanceCollateral.multipliedBy(
            new BigNumber(
                Web3.utils.fromWei(auth.contractStatusData.bitcoinPrice)
            )
        );
...
const userTGBalance = (auth) => {
    if (auth.userBalanceData && auth.contractStatusData) {
        const tgBalance = new BigNumber(
            Web3.utils.fromWei(auth.userBalanceData.mocBalance)
        );
        const tgBalanceUsd = new BigNumber(
            Web3.utils.fromWei(auth.contractStatusData.mocPrice)
        ).multipliedBy(tgBalance);
        const tgBalanceCollateral = tgBalanceUsd.div(
            new BigNumber(
                Web3.utils.fromWei(auth.contractStatusData.bitcoinPrice)
            )
        );
...
const userCollateralBalance = (auth) => {
    if (auth.userBalanceData && auth.contractStatusData) {
        const collateralBalance = new BigNumber(
            Web3.utils.fromWei(auth.userBalanceData.rbtcBalance)
        );
        const collateralBalanceUsd = new BigNumber(
            Web3.utils.fromWei(auth.contractStatusData.bitcoinPrice)
...
const getUserBalance = (auth, i18n, tokenName) => {
    ...
    if (auth.userBalanceData && auth.contractStatusData) {
        ...
        result = {
            normal: setToLocaleString(
                notFormatted.normal.toFixed(rDecimals),
                rDecimals,
                i18n
            ),
            normal_tooltip: setToLocaleString(
                notFormatted.normal.toFixed(12),
                12,
                i18n
            ),
            usd: setToLocaleString(
                notFormatted.usd.toFixed(parseInt(config.tokens.TP.decimals)),
                parseInt(config.tokens.TP.decimals),
                i18n
            ),
            usd_tooltip: setToLocaleString(
                notFormatted.usd.toFixed(12),
                12,
                i18n
            ),
            collateral: setToLocaleString(
                notFormatted.collateral.toFixed(
                    parseInt(config.tokens.RESERVE.decimals)
                ),
                parseInt(config.tokens.RESERVE.decimals),
                i18n
            ),
            collateral_tooltip: setToLocaleString(
                notFormatted.collateral.toFixed(12),
                12,
                i18n
            ),
            raw: {
