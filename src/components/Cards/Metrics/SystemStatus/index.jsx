...
function SystemStatus(props) {
    ...
    const configStatusOrange = {
        ...
        operationsAvailable: ['mintRISKPRO', 'redeemRISKPROX']
    };
    ...
    const configStatusRed = {
        ...
        operationsAvailable: ['redeemRISKPROX']
    };
    ...
    const getConfigByCoverage = (
        coverage,
        adjustedTargetCoverage,
        paused,
        blocksToSettlement,
        price_active
    ) => {
        ...
        if (
            new BigNumber(coverage).gte(new BigNumber(adjustedTargetCoverage))
        ) {
            return configStatusGreen;
        } else if (
            new BigNumber(coverage).gte(
                formatValueToContract(config.globalCoverage.warning, 'COV')
            )
        ) {
            return configStatusYellow;
        } else if (
            new BigNumber(coverage).gte(
                formatValueToContract(config.globalCoverage.dangerous, 'COV')
            )
        ) {
    ...
    const { className, operationsAvailable } = getConfigByCoverage(
        props.coverage,
        props.adjustedTargetCoverage,
        props.paused,
        props.blocksToSettlement,
        price_active
    );
    ...
    return (
        <div className="Card CardSystemStatus">
            ...
            <div
                className="CardMetricContent"
                style={{ marginTop: 0, justifyItems: 'center' }}
            >
                {!loading ? (
                    <>
                        <div>
                        ...
                        <div>
                            <h5>
                                {t('global.Metrics_globalCoverage', {
                                    ns: 'global'
                                })}
                            </h5>
                            <Tooltip
                                placement="top"
                                title={getDatas['globalCoverageTooltip']}
                            >
                                <span
                                    className={className}
                                    style={{ fontSize: 21 }}
            ...
            <h3 className="CardTitle" style={{ marginTop: 50 }}>
                {t(`${AppProject}.metrics.systemOperations.title`, { ns: ns })}
            </h3>
