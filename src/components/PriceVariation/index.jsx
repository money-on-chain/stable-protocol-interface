...
export default function PriceVariation(props) {
    ...
    const formattedVar = formatValueVariation(
        priceDiff.times(new BigNumber(10).exponentiatedBy(18)),
        i18n.languages[0],
        auth
    );
    const formattedPerc = parseFloat(
        priceDiff.div(priceVariation.day).times(100)
    ).toLocaleString(i18n.languages[0], {
    ...
    const tooltip = (
        <div className="PriceVariationTooltip">
            <p>
                {t(`${AppProject}.general.priceVariation.tooltip.titleDaily`, {
                    ns: ns
                })}
            </p>
            {auth.contractStatusData.blockHeight >= 0 && (
                <p>
                    <b>
                        {t(
                            `${AppProject}.general.priceVariation.tooltip.currentBlock`,
                            { ns: ns }
                        )}
                        :
                    </b>{' '}
            ...
            <p>
                <b>
                    {t(
                        `${AppProject}.general.priceVariation.tooltip.currentValue`,
                        { ns: ns }
                    )}
                    :
                </b>{' '}
                <LargeNumberF3
                    {...{
                        amount: getBalanceUSD(props.tokenName),
            ...
            {auth.contractStatusData.historic.blockHeight >= 0 && (
                <p>
                    <b>
                        {t(
                            `${AppProject}.general.priceVariation.tooltip.referenceBlock`,
                            { ns: ns }
                        )}
                        :
                    </b>{' '}
            ...
            <p>
                <b>
                    {t(
                        `${AppProject}.general.priceVariation.tooltip.referenceValue`,
                        { ns: ns }
                    )}
                    :
                </b>{' '}
