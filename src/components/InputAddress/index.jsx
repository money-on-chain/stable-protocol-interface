...
export default function InputAddress(props) {
    ...
    const addressOrRNSIsValid = (addressOrRNS) => {
        ...
        const isValidChecksum =
            addressOrRNS === undefined
                ? false
                : helper.isValidAddressChecksum(addressOrRNS);
        ...
        if (!isHexadecimal && rns.utils.isValidDomain(addressOrRNS)) {
            return rns
                .addr(addressOrRNS)
                .then((addr) =>
                    onResolutionCompleted(true, `Address: ${addr}`, addr)
                )
                .catch((_) =>
