...
export default function MintModal(props) {
    ...
    const renderAmount = (name, amountAndCurrencyCode, classElement) => {
        return (
            <div
                className={`AlignedAndCentered Amount mrb-0 ${classElement} ${auth.getAppMode}-${amountAndCurrencyCode.currencyCode}`}
                style={{ display: 'flex' }}
            >
    ...
    const confirmButton = async ({ comment, tolerance }) => {
        ...
        if (auth.getAppMode === 'RRC20') {
            const userAllowance = await auth.getReserveAllowance(
                window.address
            );
    ...
    const setAllowanceReserve = () => {
        ...
        const result = auth.interfaceApproveReserve(
            window.address,
            (a, _txHash) => {
    ...
    const onConfirmTransactionFinish = async () => {
        ...
        if (fee.enoughMOCBalance == true) {
            userAmount =
                userAmount -
                Web3.utils.fromWei((fee.percentage * 100).toString(), 'ether');
        }
    ...
    return (
        ...
        >
            <div className="TabularContent">
                ...
                <div className={'text-align-center'}>
                    <img width={30} height={30} src={IconDownArrow} alt="ssa" />
                </div>
                {renderAmount(
                            ...
                            <>
                                <div className="MOCFee mrb-0">
                                    ...
                                    >
                                        ...
                                        <span
                                            className={`Value ${appMode} color-08374F`}
                                        >
                                            {auth.isLoggedIn && (
                                                <LargeNumber
                                        ...
                                        if (
                                            ...
                                            return (
                                                <>
                                                    ...
                                                    >
                                                        {t(
                                                            `${AppProject}.PleaseReviewYourWallet`,
                                                            { ns: ns }
                                                        )}
                                                    </p>
                                                </>
