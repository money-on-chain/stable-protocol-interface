...
export default function SendModal(props) {
    ...
    const [tokenToSend, setTokenToSend] = useState(
        props.tokensToSend && props.tokensToSend[0]
    );
    ...
    const getMaxToSend = () => {
        const { amount } = getBalanceAndTransferMethodOfTokenToSend(
            userState.userBalanceData,
            tokenToSend
        );
        return amount;
    };
    ...
    const cancelFull = () => {
        if (confirmModal == false) {
            if (showTransactionModal) {
                if (
                    txtTransaction != 'SUCCESSFUL' &&
                    txtTransaction != 'REVIEW'
                ) {
    ...
    const getMethodTransferTo = () => {
        const { methodTransferTo } = getBalanceAndTransferMethodOfTokenToSend(
            userState.userBalanceData,
            tokenToSend,
            auth
        );
        ...
        <>
            <Button
                ...
                style={{
                    ...
                    fontSize: '1em',
                    fontWeight: view === 'moc' ? 700 : 500,
                    marginTop: view === 'moc' && '5.9em'
                }}
            >
            ...
            >
                    ...
                    {statusScreen !== 1 &&
                        ...
                        statusScreen !== 4 && (
                            <>
                                <InputAddress
                                    title={t(
                                        `${AppProject}.wallets.receiverAddress`,
                                        { ns: ns }
                                    )}
                                    value={address}
                                    onChange={(event) =>
                                            ...
                                            >
                                                ...
                                                <div style={{ float: 'right' }}>
                                                    <p className={'copy-txt'}>
                                                        ...
                                                        <span>
                                                            {t(
                                                                `${AppProject}.Tokens_${tokenToSend}_code`,
                                                                { ns: ns }
                                                            )}
                                                        </span>
                                                    </p>
                    ...
                    {statusScreen === 2 && (
                        <>
                            <div style={{ textAlign: 'center' }}>
                                ...
                                <br />
                                <br />
                                <p className={'Transaction_confirmation'}>
                                    {t(`${AppProject}.PleaseReviewYourWallet`, {
                                        ns: ns
                                    })}
                                </p>
                                <br />
                ...
                <Modal visible={confirmModal} footer={null} width={450}>
                    ...
                    />
                    <div className={'div-txt'}>
                        <p className={'color-08374F'}>
                            {t('global.ModalSend_CopyTx')}
                        </p>
                        <div>
